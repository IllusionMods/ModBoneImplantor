...
namespace ModBoneImplantor
{
	...
	public class ChaImplantManager : MonoBehaviour
	{
		...
		public bool ImplantBones(int id, string assetName, Transform src, Transform dst, Dictionary<string, GameObject> dictBone)
		{
			...
			if (dictBone.TryGetValue(dst.name, out objParent))
			{
			...
			else
			{
				...
				Logger.Log(LogLevel.Error, $"Your BoneImplantProcess is invalid. trfDst is {dst.name}.");
				Logger.Log(LogLevel.Error, $"trfDst must be the bone stored in the same structure as official body skeleton.");
				Logger.Log(LogLevel.Error, $"You cannot set your original bone or placeholder object such as cf_o_root for trfDst.");
				return false;
			}
